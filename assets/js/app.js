...
const SOURCE_PALETTE = ['#facc15','#2563eb','#22c55e','#f97316','#a855f7','#ef4444','#14b8a6','#ec4899','#84cc16','#64748b'];
...
function init() {
  ...
  ['viewMode','sortSelect','scaleRange','labelMode','colorMode','ribbonRule','similarityRange','showRibbons','showUnmatched'].forEach(id => {
  ...
  $('zoomInBtn')?.addEventListener('click', () => setMapZoom(state.mapZoom * 1.25));
  $('zoomOutBtn')?.addEventListener('click', () => setMapZoom(state.mapZoom / 1.25));
  $('zoomResetBtn')?.addEventListener('click', () => setMapZoom(1));
  $('mapPanel')?.addEventListener('wheel', handleMapWheel, { passive: false });
  installMapDragPan();
  const dz = $('dropzone');
...
function setMapZoom(value) {
  ...
  const nextZoom = Math.min(4, Math.max(0.25, Number(value) || 1));
  if (!panel) { state.mapZoom = nextZoom; return; }
  const centerX = (panel.scrollLeft + panel.clientWidth / 2) / oldZoom;
  const centerY = (panel.scrollTop + panel.clientHeight / 2) / oldZoom;
  state.mapZoom = nextZoom;
  applyMapZoom();
  panel.scrollLeft = Math.max(0, centerX * nextZoom - panel.clientWidth / 2);
  panel.scrollTop = Math.max(0, centerY * nextZoom - panel.clientHeight / 2);
}
...
function applyMapZoom() {
  ...
  const baseWidth = Number(svg.getAttribute('width')) || 900;
  const baseHeight = Number(svg.getAttribute('height')) || 520;
...
function updateZoomText() {
  const z = $('zoomValue');
  if (z) z.textContent = `${Math.round((state.mapZoom || 1) * 100)}%`;
}
...
function handleMapWheel(e) {
  ...
  const contentX = (panel.scrollLeft + pointerX) / oldZoom;
  const contentY = (panel.scrollTop + pointerY) / oldZoom;
  const factor = e.deltaY < 0 ? 1.12 : 1 / 1.12;
  state.mapZoom = Math.min(4, Math.max(0.25, oldZoom * factor));
  applyMapZoom();
  panel.scrollLeft = Math.max(0, contentX * state.mapZoom - pointerX);
  panel.scrollTop = Math.max(0, contentY * state.mapZoom - pointerY);
}

function installMapDragPan() {
  const panel = $('mapPanel');
  if (!panel) return;
  ...
  window.addEventListener('mousemove', e => {
    ...
    const dx = e.clientX - state.panDrag.x;
    const dy = e.clientY - state.panDrag.y;
    if (Math.abs(dx) + Math.abs(dy) > 3) state.panDrag.moved = true;
    panel.scrollLeft = state.panDrag.scrollLeft - dx;
    panel.scrollTop = state.panDrag.scrollTop - dy;
    e.preventDefault();
  });
  window.addEventListener('mouseup', () => {
    ...
    if (moved) {
      const stopClick = ev => { ev.preventDefault(); ev.stopPropagation(); };
...
function assignFamilies() {
  ...
  state.sourceColors = new Map(state.genomes.map((g, i) => [g.id, SOURCE_PALETTE[i % SOURCE_PALETTE.length]]));
}
...
function render() {
  ...
  if (($('viewMode')?.value || 'linear') === 'circular') renderCircular(genomes);
  else renderLinear(genomes);
  applyMapZoom();
  renderColorHelp();
  applySearch();
}

function renderLinear(genomes) {
  const svg = $('genomeMap');
  svg.replaceChildren();
  ...
  genomes.forEach((g, i) => drawGenomeLinear(svg, g, i, left, top + i * trackGap, bpPerPx, positions));
  if ($('showRibbons').checked && genomes.length > 1) drawRibbonsLinear(svg, genomes, bpPerPx, positions);
  drawSvgLegend(svg, width - 250, height - 112);
}

function renderCircular(genomes) {
  const svg = $('genomeMap');
  svg.replaceChildren();
  ...
  const outerPad = 120;
  const innerBase = 128;
  const outerRadius = innerBase + (genomes.length - 1) * ringStep + 16;
  const width = Math.max(1280, Math.ceil(outerRadius * 2 + 680));
  const height = Math.max(980, Math.ceil(outerRadius * 2 + 240));
  const cx = outerPad + outerRadius;
  const cy = height / 2;
  svg.setAttribute('viewBox', `0 0 ${width} ${height}`);
  svg.setAttribute('width', width);
  svg.setAttribute('height', height);
  ...
  [0,90,180,270].forEach(a => {
    const p1 = polar(cx, cy, 36, a);
    const p2 = polar(cx, cy, outerRadius + 16, a);
    guideLayer.appendChild(el('line', { x1: p1.x, y1: p1.y, x2: p2.x, y2: p2.y, class: 'circular-guide' }));
  });
  const startLabel = polar(cx, cy, outerRadius + 34, -90);
  guideLayer.appendChild(text(startLabel.x, startLabel.y, 'displayed start', { class: 'circular-note', 'text-anchor': 'middle' }));

  const positions = new Map();
  const labelX = cx + outerRadius + 52;
  const metadataX = labelX + 230;
  genomes.forEach((g, i) => {
    const axisRadius = innerBase + (genomes.length - 1 - i) * ringStep;
    drawGenomeCircular(svg, g, i, cx, cy, axisRadius, outerRadius, labelX, metadataX, positions);
  });
  if ($('showRibbons').checked && genomes.length > 1) drawRibbonsCircular(svg, genomes, cx, cy, positions);
  drawSvgLegend(svg, width - 250, height - 112);
}
...
function drawGenomeLinear(svg, genome, index, x, y, bpPerPx, positions) {
  ...
  group.appendChild(text(16, y - 54, genome.name, { class: 'track-label' }));
  group.appendChild(text(16, y - 35, `${formatBp(genome.length)} · ${genome.genes.length} CDS · ${genome.filename}${originText}`, { class: 'track-meta' }));
  ...
  for (const gene of genes) {
    ...
    for (const segment of segments) {
      ...
      const lane = segment.strand > 0 ? (gene.number % 2 ? -26 : -48) : (gene.number % 2 ? 26 : 48);
      const gy = y + lane;
      ...
      const labelText = resolveLinearLabel(gene, gw);
      if (labelText) {
        const lx = gx + gw / 2;
        const ly = lane < 0 ? gy - 10 : gy + 36;
        const angle = lane < 0 ? -34 : 34;
        group.appendChild(text(lx, ly, labelText, {
          class: 'gene-label',
          'text-anchor': 'middle',
...
function resolveLinearLabel(gene, widthPx) {
  const labelMode = $('labelMode').value;
  if (labelMode === 'none') return '';
  if (labelMode === 'product') return shortProduct(gene.product);
  if (labelMode === 'gene') return gene.gene;
  return widthPx > 30 ? gene.gene : '';
}

function drawGenomeCircular(svg, genome, index, cx, cy, axisRadius, outerRadius, labelX, metadataX, positions) {
  const group = el('g', { class: 'genome-track', 'data-genome': genome.id });
  const reversed = state.reversed.has(genome.id);
  const originStart = getOriginStart(genome);
  group.appendChild(el('circle', { cx, cy, r: axisRadius, class: 'circular-axis' }));

  const leaderAngle = -28;
  const anchor = polar(cx, cy, axisRadius, leaderAngle);
  const mid = { x: anchor.x + 24, y: anchor.y };
  group.appendChild(el('line', { x1: anchor.x, y1: anchor.y, x2: mid.x, y2: mid.y, class: 'ring-label-line' }));
  group.appendChild(el('line', { x1: mid.x, y1: mid.y, x2: labelX - 6, y2: mid.y, class: 'ring-label-line' }));
  group.appendChild(text(labelX, mid.y - 4, genome.name, { class: 'ring-label', 'text-anchor': 'start' }));
  group.appendChild(text(metadataX, mid.y - 4, `${formatBp(genome.length)} · ${genome.genes.length} CDS${originStart > 1 ? ` · start ${formatBp(originStart)} at 12 o'clock` : ''}`, { class: 'ring-label-meta', 'text-anchor': 'start' }));
  ...
  for (const gene of genes) {
    ...
    const drawn = [];
    for (const segment of segments) {
      const a0 = circleAngleForBp(segment.start, genome.length);
      const a1 = circleAngleForBp(segment.end + 1, genome.length);
      const band = circularBandForStrand(axisRadius, segment.strand);
      const pts = annularArrowPoints(cx, cy, band.inner, band.outer, a0, a1, segment.strand);
      const poly = el('polygon', {
        points: pts,
        fill: geneColor(gene, genome),
        class: 'gene-arrow',
        'data-search': `${genome.name} ${gene.gene} ${gene.locusTag} ${gene.product} ${gene.family} ${gene.proteinId}`.toLowerCase(),
        'data-family': gene.family
      });
      poly.addEventListener('click', () => showGeneDetails(genome, gene));
      poly.addEventListener('mousemove', e => showTooltip(e, `${escapeHtml(genome.name)}<br><strong>${escapeHtml(gene.gene)}</strong> ${escapeHtml(gene.product)}<br>${gene.start}-${gene.end} ${gene.strand > 0 ? '+' : '-'} · displayed origin ${formatBp(originStart)} · ${escapeHtml(gene.family)}`));
      poly.addEventListener('mouseleave', hideTooltip);
      group.appendChild(poly);

      drawn.push({ a0, a1, r: axisRadius, bandMid: (band.inner + band.outer) / 2 });
    }
    if (drawn.length) {
      const primary = drawn.sort((a,b) => (b.a1 - b.a0) - (a.a1 - a.a0))[0];
      positions.set(keyFor(genome, gene), primary);
    }
  }
...
function circularBandForStrand(axisRadius, strand) {
  if (strand > 0) return { inner: axisRadius + 3, outer: axisRadius + 13 };
  return { inner: axisRadius - 13, outer: axisRadius - 3 };
}

function circleAngleForBp(bp, length) {
  return -90 + ((bp - 1) / Math.max(1, length)) * 360;
}

function annularArrowPoints(cx, cy, ri, ro, a0, a1, strand) {
  const total = Math.max(0.8, a1 - a0);
  const head = Math.min(10, Math.max(1.5, total * 0.34));
  let pts = [];
  if (strand > 0) {
    const bodyEnd = Math.max(a0 + 0.4, a1 - head);
    const outerPts = sampleArc(cx, cy, ro, a0, bodyEnd);
    const innerPts = sampleArc(cx, cy, ri, bodyEnd, a0);
    const tip = polar(cx, cy, (ri + ro) / 2, a1);
    pts = [...outerPts, tip, ...innerPts];
  } else {
    const bodyStart = Math.min(a1 - 0.4, a0 + head);
    const outerPts = sampleArc(cx, cy, ro, bodyStart, a1);
    const innerPts = sampleArc(cx, cy, ri, a1, bodyStart);
    const tip = polar(cx, cy, (ri + ro) / 2, a0);
    pts = [tip, ...outerPts, ...innerPts];
  }
  return pts.map(p => `${p.x},${p.y}`).join(' ');
}

function sampleArc(cx, cy, r, startDeg, endDeg) {
  const diff = endDeg - startDeg;
  const steps = Math.max(2, Math.ceil(Math.abs(diff) / 5));
  const out = [];
  for (let i = 0; i <= steps; i++) {
    const a = startDeg + (diff * i / steps);
    out.push(polar(cx, cy, r, a));
  }
  return out;
}
...
function anchorFromAngle(angleDeg) {
  const n = ((angleDeg % 360) + 360) % 360;
  if (n > 90 && n < 270) return 'end';
  return 'start';
}

function drawRibbonsLinear(svg, genomes, bpPerPx, positions) {
  ...
  for (let i = 0; i < genomes.length - 1; i++) {
    ...
    for (const m of matches) {
      ...
      if (!pa || !pb) continue;
      const path = ribbonPathLinear(pa.x1, pa.x2, pa.y, pb.x1, pb.x2, pb.y);
...
function drawRibbonsCircular(svg, genomes, cx, cy, positions) {
  const layer = el('g', { class: 'ribbons' });
  svg.insertBefore(layer, svg.firstChild.nextSibling);
  const minSim = Number($('similarityRange').value) / 100;
  const rule = $('ribbonRule').value;
  for (let i = 0; i < genomes.length - 1; i++) {
    ...
    for (const m of matches) {
      ...
      const pb = positions.get(keyFor(b, m.b));
      if (!pa || !pb) continue;
      const path = ribbonPathCircular(cx, cy, pa, pb);
      const r = el('path', { d: path, class: 'ribbon', fill: geneColor(m.a, a), 'data-search': `${a.name} ${b.name} ${m.a.product} ${m.b.product} ${m.a.family}`.toLowerCase() });
      r.addEventListener('mousemove', e => showTooltip(e, `<strong>${escapeHtml(m.a.family)}</strong><br>${escapeHtml(a.name)}: ${escapeHtml(m.a.gene)}<br>${escapeHtml(b.name)}: ${escapeHtml(m.b.gene)}<br>score: ${Math.round(m.score*100)}%`));
      r.addEventListener('mouseleave', hideTooltip);
      layer.appendChild(r);
...
function ribbonPathLinear(x1, x2, y1, x3, x4, y2) {
  const c = Math.max(28, Math.abs(y2 - y1) * .42);
  return `M ${x1} ${y1} C ${x1} ${y1 + c}, ${x3} ${y2 - c}, ${x3} ${y2} L ${x4} ${y2} C ${x4} ${y2 - c}, ${x2} ${y1 + c}, ${x2} ${y1} Z`;
}

function ribbonPathCircular(cx, cy, pa, pb) {
  // Connect adjacent circular rings with a quadrilateral whose two outer edges
  // follow the corresponding angular spans. The previous cubic-Bezier version
  // pulled control points toward a shared mid-radius and made inner ribbons
  // visually collapse toward the centre line for some angle combinations.
  const outer = pa.r >= pb.r ? pa : pb;
  const inner = pa.r >= pb.r ? pb : pa;
  const outerStart = polar(cx, cy, outer.r, outer.a0);
  const outerEnd = polar(cx, cy, outer.r, outer.a1);
  const innerStart = polar(cx, cy, inner.r, inner.a0);
  const innerEnd = polar(cx, cy, inner.r, inner.a1);

  const outerArc = svgArcPath(cx, cy, outer.r, outer.a0, outer.a1);
  const innerArc = svgArcPath(cx, cy, inner.r, inner.a1, inner.a0);
...
function geneColor(gene, genome) {
  const override = state.geneOverrides.get(keyFor(genome, gene));
...
function drawSvgLegend(svg, x, y) {
  ...
  let heading;
  if (mode === 'product') {
    entries = [...PRODUCT_COLORS.entries()].slice(0, 8);
    heading = 'Product color hints';
  } else if (mode === 'source') {
    entries = orderedGenomes().map(g => [g.name, state.sourceColors.get(g.id) || '#64748b']).slice(0, 8);
    heading = 'Genome colors';
  } else if (mode === 'strand') {
    entries = [['Forward strand', '#2563eb'], ['Reverse strand', '#dc2626']];
    heading = 'Gene direction';
  } else {
    entries = [...state.familyColors.entries()].filter(([f]) => (state.families.get(f)||0) > 1).slice(0, 8);
    heading = 'Shared predicted families';
  }
  group.appendChild(text(x, y - 12, heading, { class: 'track-meta' }));
...
function renderColorHelp() {
  ...
  if (mode === 'family') {
    ...
    intro = 'Each genome has one editable color. Manual gene colors override this, so selected genes such as integrase can be highlighted separately.';
...
function renderGenomeList() {
  ...
  const viewMode = $('viewMode')?.value || 'linear';
  orderedGenomes().forEach(g => {
    ...
    colorControls.innerHTML = `
      <label>Genome color
        <input type="color" value="${escapeHtml(state.sourceColors.get(g.id) || '#64748b')}" aria-label="Genome color for ${escapeHtml(g.name)}">
      </label>
      <span class="muted small">Used when “Color genes by” is set to Genome.</span>`;
    colorControls.querySelector('input[type="color"]').addEventListener('change', e => {
      state.sourceColors.set(g.id, e.target.value);
      render();
    });
    div.appendChild(colorControls);
    ...
    controls.innerHTML = `
      ...
      <div class="origin-note">${originStart === 1 ? 'Original GenBank coordinate order.' : viewMode === 'circular' ? `Original bp ${formatBp(originStart)} is displayed at 12 o'clock.` : `Original bp ${formatBp(originStart)} is displayed at the left edge.`}</div>`;
...
function showGeneDetails(genome, gene) {
  const geneKey = keyFor(genome, gene);
  const manualColor = state.geneOverrides.get(geneKey) || '';
  const currentColor = geneColor(gene, genome);
  $('geneDetails').innerHTML = `
    ...
    <strong>Displayed color:</strong> <span class="legend-chip inline-chip" style="background:${escapeHtml(currentColor)}"></span>${manualColor ? ' manual gene override' : ' current color mode'}<br>
    <div class="manual-color-row">
      <label>Manual gene color <input id="manualGeneColor" type="color" value="${escapeHtml(manualColor || currentColor)}"></label>
      <button id="applyGeneColorBtn" class="secondary">Apply to this gene</button>
      <button id="clearGeneColorBtn" class="secondary" ${manualColor ? '' : 'disabled'}>Clear manual color</button>
    </div>
  ...
  `;
  $('applyGeneColorBtn').addEventListener('click', () => {
    state.geneOverrides.set(geneKey, $('manualGeneColor').value);
    render();
    showGeneDetails(genome, gene);
  });
  $('clearGeneColorBtn').addEventListener('click', () => {
    state.geneOverrides.delete(geneKey);
    render();
    showGeneDetails(genome, gene);
  });
}
...
function exportSvgCss() {
  return `
    ...
    .ring-label { font: 700 12px Inter, Arial, sans-serif; fill: #111827; }
    .ring-label-meta { font: 10px Inter, Arial, sans-serif; fill: #64748b; }
    .axis-line { stroke: #9aa8ba; stroke-width: 2; }
    .circular-axis { fill: none; stroke: #a7b3c2; stroke-width: 1.5; }
    .circular-guide { fill: none; stroke: #e3e9f2; stroke-width: 1; stroke-dasharray: 4 5; }
    .ring-label-line { stroke: #c8d2df; stroke-width: 1.2; }
    .circular-note { font: 10px Inter, Arial, sans-serif; fill: #7b8798; }
    ...
    .gene-label { font: 10px Inter, Arial, sans-serif; fill: #111827; paint-order: stroke; stroke: rgba(255,255,255,.85); stroke-width: 3px; stroke-linecap: round; stroke-linejoin: round; }
