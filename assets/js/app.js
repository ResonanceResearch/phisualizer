...
function renderCircular(genomes) {
  ...
  const width = Math.max(1120, Math.ceil(outerRadius * 2 + 520));
  ...
  const centerLabel = el('g', { class: 'circular-center-label' });
  centerLabel.appendChild(text(cx, cy - 10, 'Circular comparison', { class: 'ring-label', 'text-anchor': 'middle' }));
  centerLabel.appendChild(text(cx, cy + 8, `${genomes.length} genomes · adjacent similarity ribbons`, { class: 'ring-label-meta', 'text-anchor': 'middle' }));
  ...
  genomes.forEach((g, i) => {
    ...
    drawGenomeCircular(svg, g, i, cx, cy, axisRadius, outerRadius, labelX, positions);
  });
...
function drawGenomeCircular(svg, genome, index, cx, cy, axisRadius, outerRadius, labelX, positions) {
  const group = el('g', { class: 'genome-track', 'data-genome': genome.id });
  const reversed = state.reversed.has(genome.id);
  const originStart = getOriginStart(genome);
  ...
  group.appendChild(text(labelX, mid.y - 7, genome.name, { class: 'ring-label', 'text-anchor': 'start' }));
  group.appendChild(text(cx, cy + 30 + index * 14, `${genome.name}: ${formatBp(genome.length)} · ${genome.genes.length} CDS${originStart > 1 ? ` · start ${formatBp(originStart)} at 12 o'clock` : ''}`, { class: 'ring-label-meta', 'text-anchor': 'middle' }));
...
function ribbonPathCircular(cx, cy, pa, pb) {
  const p1s = polar(cx, cy, pa.r, pa.a0);
  const p1e = polar(cx, cy, pa.r, pa.a1);
  const p2s = polar(cx, cy, pb.r, pb.a0);
  const p2e = polar(cx, cy, pb.r, pb.a1);
  const midR = (pa.r + pb.r) / 2;
  const c1s = polar(cx, cy, midR, pa.a0);
  const c2s = polar(cx, cy, midR, pb.a0);
  const c1e = polar(cx, cy, midR, pa.a1);
  const c2e = polar(cx, cy, midR, pb.a1);
  return `M ${p1s.x} ${p1s.y} C ${c1s.x} ${c1s.y}, ${c2s.x} ${c2s.y}, ${p2s.x} ${p2s.y} L ${p2e.x} ${p2e.y} C ${c2e.x} ${c2e.y}, ${c1e.x} ${c1e.y}, ${p1e.x} ${p1e.y} Z`;
}
