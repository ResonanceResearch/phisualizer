...
function render() {
  ...
  const left = 190, right = 80, top = 118, trackGap = 170;
...
function drawGenome(svg, genome, index, x, y, bpPerPx, positions) {
  ...
  for (const gene of genes) {
    ...
    for (const segment of segments) {
      ...
      const lane = segment.strand > 0 ? (gene.number % 2 ? -30 : -64) : (gene.number % 2 ? 30 : 64);
      const gy = y + lane;
      ...
      if (label) group.appendChild(text(gx + gw / 2, gy + (lane < 0 ? -4 : 36), label, { class: 'gene-label', 'text-anchor': 'middle' }));
    }
    if (drawn.length) {
