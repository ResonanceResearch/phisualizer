...
function render() {
  ...
  const svg = $('genomeMap');
  svg.replaceChildren();
  if (!genomes.length) { svg.setAttribute('height', '520'); return; }
  const bpPerPx = Number($('scaleRange').value);
  const left = 190, right = 60, top = 70, trackGap = 170;
  ...
  drawLegend(svg, width - 220, height - 100);
  applySearch();
}
...
function drawGenome(svg, genome, index, x, y, bpPerPx, positions) {
  ...
  group.appendChild(text(16, y - 36, genome.name, { class: 'track-label' }));
  group.appendChild(text(16, y - 18, `${formatBp(genome.length)} · ${genome.genes.length} CDS · ${genome.filename}`, { class: 'track-meta' }));
  ...
  for (const gene of genes) {
    ...
    const dispStart = reversed ? genome.length - trueEnd + 1 : trueStart;
    const dispEnd = reversed ? genome.length - trueStart + 1 : trueEnd;
    const strand = reversed ? -gene.strand : gene.strand;
    const gx = x + dispStart / bpPerPx;
    const gw = Math.max(5, (dispEnd - dispStart + 1) / bpPerPx);
    const lane = strand > 0 ? (gene.number % 2 ? -30 : -64) : (gene.number % 2 ? 30 : 64);
    const gy = y + lane;
    const arrow = geneArrow(gx, gy, gw, 24, strand, geneColor(gene, genome));
    ...
    arrow.addEventListener('mousemove', e => showTooltip(e, `${escapeHtml(genome.name)}<br><strong>${escapeHtml(gene.gene)}</strong> ${escapeHtml(gene.product)}<br>${gene.start}-${gene.end} ${gene.strand > 0 ? '+' : '-'} · ${escapeHtml(gene.family)}`));
    arrow.addEventListener('mouseleave', hideTooltip);
    group.appendChild(arrow);
    positions.set(keyFor(genome, gene), { x1: gx, x2: gx + gw, y: gy + (lane < 0 ? 24 : 0), cx: gx + gw/2 });
    ...
    if (label) {
      group.appendChild(text(gx + gw / 2, gy + (lane < 0 ? -4 : 36), label, { class: 'gene-label', 'text-anchor': 'middle' }));
    }
  }
...
function showGeneDetails(genome, gene) {
  $('geneDetails').innerHTML = `
    ...
    <code>${escapeHtml(gene.location)}</code> (${gene.strand > 0 ? 'forward' : 'reverse'})<br>
...
function exportSvg() {
  const svg = $('genomeMap').cloneNode(true);
...
function exportPng() {
  ...
  img.onload = () => {
    ...
    canvas.width = img.width || Number(svg.getAttribute('width'));
    canvas.height = img.height || Number(svg.getAttribute('height'));
    const ctx = canvas.getContext('2d');
    ctx.fillStyle = '#ffffff'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0);
    canvas.toBlob(blob => downloadBlob(blob, 'phage-map.png'));
