...
function renderCircular(genomes) {
  ...
  ['displayed start', '¼ genome', '½ genome', '¾ genome'].forEach((label, i) => {
    const a = -90 + i * 90;
    const p = polar(cx, cy, outerRadius + 34, a);
    guideLayer.appendChild(text(p.x, p.y, label, { class: 'circular-note', 'text-anchor': anchorFromAngle(a) }));
  });
...
function drawGenomeCircular(svg, genome, index, cx, cy, axisRadius, outerRadius, labelX, positions) {
  ...
  group.appendChild(text(labelX, mid.y + 10, `${formatBp(genome.length)} · ${genome.genes.length} CDS${originStart > 1 ? ` · start ${formatBp(originStart)} at 12 o'clock` : ''}`, { class: 'ring-label-meta', 'text-anchor': 'start' }));
